...
public enum RoleEnum {
    ...
    public static Optional<RoleEnum> getByRoleName(final String roleName) {
        return Optional.ofNullable(roleName).map(String::toUpperCase).flatMap(
                name -> Arrays.stream(RoleEnum.values()).filter(role -> role.getRoleName().toUpperCase().equals(name))
                        .findFirst());
    }
